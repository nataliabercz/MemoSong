...
class MemoSong(GenericFunctions):
    ...
    def open_note(self) -> None:
        ...
        if file_name := self._get_curselection_from_radiobutton_list(self.notes_radiobutton_list):
            ...
            try:
                with open(f'{self.app_path}/notes/{file_name}') as file:
                    for line in file:
                        self.notepad_text_area.insert(tkinter.END, line)
                    file.close()
                self.notepad_title_field.insert(0, ''.join(file_name.split('.')[0:-1]))
    ...
    def edit_file(self, option: str) -> None:
        # current browser type - why do not use it globally? what's going on here?
        if browser_type := self.current_browser_type:
            file_to_edit = self._get_curselection_from_radiobutton_list(getattr(self,
                                                                                f'{browser_type}_radiobutton_list'))
            self.rename_file(browser_type, file_to_edit) if option == 'rename' \
                else self.delete_file(browser_type, file_to_edit)
        else:
            self._display_message_box('ERROR', f'Please, select a file to {option}', False, 300)

    def rename_file(self, browser_type: str, file_to_rename: str) -> None:
        try:
            new_name = customtkinter.CTkInputDialog(text='New Name', title='RENAME FILE').get_input()
            extension = self._get_extension(browser_type)
            if new_name:
                self._mute_playback()
                os.rename(f'{self.app_path}/{browser_type}/{file_to_rename}',
                          f'{self.app_path}/{browser_type}/{new_name}.{extension}')
                self.update_list(browser_type)
            if browser_type == 'notes':
                self.notepad_title_field.delete(0, tkinter.END)
    ...
    def delete_file(self, browser_type: str, file_to_delete: str) -> None:
        try:
            msg = self._display_message_box('DELETE FILE', f'Do you want to delete\n{file_to_delete}?', True)
            if msg.get() == 'Yes':
                self._mute_playback()
                os.remove(f'{self.app_path}/{browser_type}/{file_to_delete}')
                self.update_list(browser_type)
                if browser_type == 'notes':
                    self.notepad_text_area.delete('1.0', tkinter.END)
                    self.notepad_title_field.delete(0, tkinter.END)
