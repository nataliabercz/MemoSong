...
class MemoSong(GenericFunctions):
    ...
    def configure_window(self) -> None:
        ...
        self.root.configure()
        left = (self.root.winfo_screenwidth() - self._width) / 2
        top = (self.root.winfo_screenheight() - self._height) / 2
        self.root.geometry('%dx%d+%d+%d' % (self._width, self._height, left, top))
        self.root.resizable(0, 0)
        self.root.columnconfigure(0, weight=1)

    def configure_logo(self) -> None:
        logo_frame = self._create_main_frame(0, 0, sticky=tkinter.NW, frame_data={'width': int(self._width/7),
                                                                                  'height': int(self._height/4)})
    ...
    def configure_voice_recorder(self) -> None:
        microphone_frame = self._create_main_frame(1, 0, sticky=tkinter.NW, frame_data={'width': int(self._width/4),
                                                                                        'height': int(self._height/4)})
    ...
    def configure_control_panel(self) -> None:
        control_panel_frame = self._create_main_frame(0, 1, frame_data={'width': int(self._width/4),
                                                                        'height': int(self._height/3)},
    ...
    def add_piano_recorder_title_frame(self, frame: customtkinter.CTkFrame) -> None:
        title_frame = customtkinter.CTkFrame(frame, height=int(self._height/12))
        ...
        self._create_button_from_image(title_frame, 'keyboard', (45, 25), self._turn_keyboard_piano, tkinter.LEFT, self)
        self.root.bind('`', self._turn_keyboard_piano)
        self._create_button_from_image(title_frame, 'mute', (25, 25), self._mute_playback, tkinter.RIGHT, self)

    def _turn_keyboard_piano(self, _: Any = None) -> None:
    ...
    def _play_pressed_key(self, event) -> None:
    ...
    def _play_key(self, key: str) -> None:
        Piano.key = key
        self._start_new_thread(self.piano.play_key)
    ...
    def _create_piano_button(self, frame: customtkinter.CTkFrame, key: str, pad_x: int, pad_y: int, span: int,
                             **kwargs: Any) -> None:
        button = customtkinter.CTkButton(frame, command=lambda x=key: self._play_key(x), text='', corner_radius=0,
                                         **kwargs)
        button.grid(column=0, row=2, columnspan=span, sticky=tkinter.NW, padx=pad_x, pady=pad_y)
        self._add_key_names(button, key)
